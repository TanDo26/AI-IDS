...
def _get_meta_columns(config) :
    ...
    return [ds_cfg["target_column"], "BinaryLabel", "OriginalLabel", "_source_file"]
...
def temporal_split(df, config):
    ...
    y_test = df_test["BinaryLabel"].reset_index(drop=True)

    val_frac = temp_cfg["val_fraction_from_train"]

    X_train_full = df_train_full[feature_cols]
    y_train_full = df_train_full["BinaryLabel"]
...
def save_split(X_train, X_val, X_test, y_train, y_val, y_test, split_name, config):
    ...
    out_dir = Path(
        config["output"]["splits_dir"].format(
    ...
    X_train.to_parquet(
        split_dir / "X_train.parquet",
        index=False,
    )

    X_val.to_parquet(
        split_dir / "X_val.parquet",
        index=False,
    )
    ...
    y_train.to_frame("BinaryLabel").to_parquet(
        split_dir / "y_train.parquet",
        index=False,
    )

    y_val.to_frame("BinaryLabel").to_parquet(
        split_dir / "y_val.parquet",
        index=False,
    )

    y_test.to_frame("BinaryLabel").to_parquet(
